...
def save_graphs(all_graphs, epoch, save_dir="./"):
...
def test(args, model, device, train_graphs, test_graphs, epoch,SG_model):
    ...
    for graph in train_graphs:
        edge = SG_model(graph)
        graph.edge_mat = edge
        all_graphs.append(graph)  # 将训练集图存储到 all_graphs 中
        # 存一下这个graph
    ...
    for graph in test_graphs:
        ...
        all_graphs.append(graph)  # 将训练集图存储到 all_graphs 中
        # 存一下这个graph
    # 假设 all_graphs 已经填充了所有图
    save_graphs(all_graphs, epoch)  # 将图保存到当前路径，并包含epoch
...
parser.add_argument("--mi_weight", type=float, default=0.01, help="classifier hidden dims")
...
# graph_filename = "./data/adhd.mat"
graph1 = loadmat(graph_filename)
graphs= load_data(graph1)

##10-fold cross validation. Conduct an experiment on the fold specified by args.fold_idx.
train_graphs, test_graphs = separate_data(graphs, test_size=0.8, random_state=42)
num_classes=2
